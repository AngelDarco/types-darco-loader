...
import Resizeobserver from "../../utils/Resizeobsever.ts";
...
const Header = () => {
  ...
  useEffect(() => {
    ...
    if (!container) return;
    observer.observer(container, (entry) => {
  ...
  return (
    <header
      ...
      className={`${styles.headerContainer} w-full h-[var(--header-height)] flex items-center justify-between px-12 border-b border-b-[var(--color)] sticky top-0 left-0 z-[100] backdrop-blur-sm`}
    >
    ...
    `}
      ...
      {!links ? (
        <div className="hidden laptop:flex absolute top-auto right-8 z-[200] ">
          {openMenu ? (
            <div className="w-8 h-8" onClick={handlerMenu}>
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth="0"
      ...
      <div
        ...
        className="w-8 h-full mr-16 flex items-center"
      >
        <Darkmode
          switchMode="static"
          hover
